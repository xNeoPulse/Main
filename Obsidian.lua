...
function Library:AddDraggableToggle(Image: string, Click, Double)
	local Table = {}
	...
	local ClickStamp = nil
	Button.MouseButton1Click:Connect(function()
		if ClickStamp and tick() - ClickStamp < 0.25 then
			Library:SafeCallback(Double, Table)
			ClickStamp = nil
			return
		end
		
		Library:SafeCallback(Click, Table)
		ClickStamp = tick()
	end)
	Library:MakeDraggable(Button, Button, true)

	Table.Button = Button
	Table.ImageLabel = ImageLabel

	function Table:SetImage(Image: string)
...
function Library:CreateWindow(WindowInfo)
	...
	if Library.IsMobile then
		...
		if WindowInfo.Icon then
			ToggleButton = Library:AddDraggableToggle(WindowInfo.Icon,
				function() Library:Toggle() end,
				function(self)
					Library.CantDragForced = not Library.CantDragForced
					self.ImageLabel.ImageTransparency = (Library.CantDragForced and 0.5 or 0)
				end
			)
		else
